...
//   const { username } = useParams<{ username: string }>()
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        setLoading(true) //write code for: instead of setLoading a gradient line along the length of top of screen shows loading 
  ...
  if (error) {
    return <div className="relative text-center mt-10 text-red-500">{error}</div>
  }
  ...
  return (
    <div className="relative p-6 justify-center flex flex-col md:flex-row">
      ...
      <div className="bg-gray-200/40 md:w-[35%] p-8 max-h-fit rounded-lg shadow-md">
        ...
        <div>
          <h1 className="text-3xl pl-0 p-4 font-bold text-gray-800">{profile.username}</h1>
          <p className="text-gray-600 leading-5 text-justify">{profile.bio || "This user has no bio."}</p>
        </div>
      </div>
      ...
      <div className="mt-8 md:mt-0 md:m-8">
        ...
        {profile.blogs.length > 0 ? (
          ...
          <p className="text-gray-600">This user has not published any blogs yet.</p>
        )}
      </div>
    </div>
  )
